...
#### قانون گروه‌بندی (مربع‌ها)
1. **هر تشخیص OCR = یک مربع**
2. **فقط وقتی مربع‌ها واقعاً به هم بخورند** (هم‌پوشانی هندسی با حاشیه کم) → یک متن حساب می‌شوند
3. نزدیک بودن بدون برخورد مربع → جدا می‌مانند
4. **اولویت مهم**: اگر یک SFX یا PROMO نزدیک یک دیالوگ (TXT) باشد → به TXT تبدیل می‌شود و در صورت برخورد مربع با آن ادغام می‌شود  
   مثال: `RIGHT` (sfx) + `NOW-` (txt) → هر دو TXT و یک متن  
   مثال: promo نزدیک dialogue → TXT
5. عنوان فصل (مثل `75화`) و کلمات کوتاه نویزی (مثل `of`) به‌ترتیب promo و junk می‌شوند
