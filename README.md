...
2. صفحهٔ اول (کاور) جدا می‌ماند؛ بقیهٔ صفحات به یک نوار بلند چسبانده می‌شوند و با **برش امن** (دور از متن/حباب) به سقف ۸۰۰۰px تقسیم می‌شوند
...
| **Stitch + برش امن** | همه صفحات (به‌جز کاور) چسبانده می‌شوند؛ برش ۸۰۰۰px روی ناحیه‌ای بدون متن/حباب انجام می‌شود |
...
| `--stitch-short-threshold` | (سازگاری) صفحاتی کوتاه‌تر از این ارتفاع؛ در نسخهٔ فعلی همه صفحات به‌جز کاور چسبانده می‌شوند | `8000` |
...
3. برش نوار به تکه‌های حداکثر `--stitch-max-height` (پیش‌فرض ۸۰۰۰px) با **خط برش امن**:
